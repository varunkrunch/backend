...
async def send_message(
    ...
    try:
        ...
        try:
            # Fetch sources and notes for the notebook to provide context
            sources_context = ""
            notes_context = ""
            try:
                # Use the same logic as the sources router directly
                logger.info(f"Attempting to fetch sources for notebook: {notebook_id}")
                
                # First, get the notebook ID if we were given a name
                actual_notebook_id = notebook_id
                if not notebook_id.startswith('notebook:'):
                    # We were given a name, need to get the ID
                    ...
                    if notebook_result and len(notebook_result) > 0:
                        ...
                        logger.info(f"Found notebook ID: {actual_notebook_id} for name: {notebook_id}")
                    else:
                        sources_context = f"Notebook '{notebook_id}' not found."
                        logger.warning(f"Notebook '{notebook_id}' not found")
                        raise Exception(f"Notebook '{notebook_id}' not found")
                else:
                    logger.info(f"Using provided notebook ID: {notebook_id}")
